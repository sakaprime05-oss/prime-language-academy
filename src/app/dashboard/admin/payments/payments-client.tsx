...
function statusClass(status: string) {
  if (status === "VERIFYING") return "border-amber-400/25 bg-amber-400/10 text-amber-700 dark:text-amber-300";
  if (status === "COMPLETED") return "border-emerald-400/25 bg-emerald-400/10 text-emerald-700 dark:text-emerald-300";
  if (status === "FAILED") return "border-red-400/25 bg-red-400/10 text-red-700 dark:text-red-300";
...
export default function AdminPaymentsClient({ initialTransactions }: { initialTransactions: Transaction[] }) {
  ...
  const stats = useMemo(() => {
    ...
    const completedAmount = initialTransactions.filter((tx) => tx.status === "COMPLETED").reduce((sum, tx) => sum + tx.amount, 0);
  ...
  return (
    <div className="space-y-5">
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="A verifier" value={stats.verifying.toString()} tone="text-amber-700 dark:text-amber-300" />
        <StatCard label="En attente" value={stats.pending.toString()} tone="text-[var(--foreground)]" />
        <StatCard label="Deja encaisse" value={money(stats.completedAmount)} tone="text-emerald-700 dark:text-emerald-300" />
        <StatCard label="Rejetes" value={stats.failed.toString()} tone="text-red-700 dark:text-red-300" />
      </section>
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm">
        <div className="flex flex-col gap-4 border-b border-[var(--border)] p-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-lg font-black text-[var(--foreground)]">Transactions recentes</h2>
        ...
        <div className="grid gap-3 p-3 lg:hidden">
          {transactions.length === 0 ? (
            ...
            transactions.map((tx) => (
              <PaymentCard key={tx.id} tx={tx} loadingId={loadingId} onApprove={handleApprove} onReject={handleReject} />
            ))
          )}
        </div>
        ...
        <div className="hidden overflow-x-auto lg:block">
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] text-[10px] font-black uppercase tracking-[0.2em] text-[var(--muted-foreground)]">
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {transactions.length === 0 ? (
                ...
                transactions.map((tx) => (
                  <PaymentRow key={tx.id} tx={tx} loadingId={loadingId} onApprove={handleApprove} onReject={handleReject} />
                ))
              )}
            </tbody>
          </table>
...
function StatCard({ label, value, tone }: { label: string; value: string; tone: string }) {
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
      <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[var(--muted-foreground)]">{label}</p>
...
function EmptyState() {
  return (
    <div className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--muted)] p-8 text-center">
      <p className="text-sm font-bold text-[var(--muted-foreground)]">Aucune transaction dans cette vue.</p>
    </div>
  );
}

function PaymentRow({ tx, loadingId, onApprove, onReject }: PaymentActionProps) {
  return (
    <tr className="transition-colors hover:bg-[var(--muted)]/60">
      <td className="whitespace-nowrap px-5 py-5 font-bold text-[var(--muted-foreground)]">{formatDate(tx.date)}</td>
      ...
      <td className="whitespace-nowrap px-5 py-5 text-base font-black text-[var(--foreground)]">{money(tx.amount)}</td>
      <td className="px-5 py-5">
...
function PaymentCard({ tx, loadingId, onApprove, onReject }: PaymentActionProps) {
  return (
    <article className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
      ...
      <div className="mt-4 grid gap-3 rounded-xl bg-[var(--muted)] p-3">
        <PaymentDetails tx={tx} />
        <div className="flex items-center justify-between border-t border-[var(--border)] pt-3">
          <span className="text-xs font-bold text-[var(--muted-foreground)]">{formatDate(tx.date)}</span>
          <span className="text-lg font-black text-[var(--foreground)]">{money(tx.amount)}</span>
        </div>
      </div>
      <div className="mt-4">
...
function PaymentDetails({ tx }: { tx: Transaction }) {
  ...
  return (
    <div className="min-w-0 space-y-1.5">
      ...
      <p className="text-xs font-semibold text-[var(--muted-foreground)]">Expediteur : {tx.senderPhone || "-"}</p>
      {proofIsUpload ? (
        <a href={tx.proof || "#"} target="_blank" rel="noopener noreferrer" className="inline-flex text-xs font-black text-sky-600 hover:underline dark:text-sky-300">
...
function PaymentActions({ tx, loadingId, onApprove, onReject, fullWidth }: PaymentActionProps) {
  if (tx.status !== "VERIFYING") {
    return <span className="text-xs font-bold text-[var(--muted-foreground)]">-</span>;
  }
  ...
  return (
    <div className={`flex gap-2 ${fullWidth ? "w-full" : "justify-end"}`}>
      <button
        ...
        disabled={disabled}
        className={`${fullWidth ? "flex-1" : ""} rounded-xl border border-red-400/25 bg-red-400/10 px-4 py-2 text-xs font-black text-red-700 transition-colors hover:bg-red-500 hover:text-white disabled:opacity-50 dark:text-red-200`}
      >
        Rejeter
      </button>
      <button
