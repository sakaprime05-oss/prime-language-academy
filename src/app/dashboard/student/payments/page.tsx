...
export default async function StudentPaymentsPage({ searchParams }: { searchParams?: Promise<PaymentSearchParams> }) {
  ...
  return (
    <div className="platform-page animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <PaymentStatusMessage status={params?.status} locked={params?.locked} />
      ...
      <section className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className={`glass-card !p-4 sm:!p-5 border-primary/20 ${paymentPlan.amountPaid > 0 ? "bg-emerald-500/5 border-emerald-500/20" : "bg-primary/[0.03]"}`}>
          ...
          <h3 className="mt-2 text-xl font-black text-[var(--foreground)]">{halfAmount.toLocaleString("fr-FR")} FCFA</h3>
          ...
          <div className="relative z-10 space-y-6">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="text-2xl sm:text-3xl font-black text-[var(--foreground)]">{remaining.toLocaleString("fr-FR")} FCFA</h3>
            ...
            <div className="space-y-3 border-t border-[var(--foreground)]/10 pt-4 text-sm font-bold">
              <div className="flex justify-between">
                ...
                <span>{paymentPlan.totalAmount.toLocaleString("fr-FR")} FCFA</span>
              </div>
              <div className="flex justify-between">
                <span className="opacity-40">Total versé</span>
                <span className="text-emerald-500">{paymentPlan.amountPaid.toLocaleString("fr-FR")} FCFA</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card !p-0 overflow-hidden">
          {paymentPlan.transactions.length === 0 ? (
            ...
            <div className="divide-y divide-[var(--foreground)]/5">
              {paymentPlan.transactions.map((transaction) => (
                <div key={transaction.id} className="flex flex-col gap-3 p-4 transition-colors hover:bg-[var(--foreground)]/[0.02] sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex gap-4 items-center">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${transaction.status === "COMPLETED" ? "bg-emerald-500/10 text-emerald-500" : transaction.status === "PENDING" ? "bg-amber-500/10 text-amber-500" : "bg-red-500/10 text-red-500"}`}>
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-sm font-black text-[var(--foreground)]">{transaction.amount.toLocaleString("fr-FR")} FCFA</p>
                      <p className="text-[10px] font-black uppercase text-[var(--foreground)]/30 tracking-widest">
                        {paymentMethodLabel(transaction.provider || transaction.method)} - {new Date(transaction.date).toLocaleDateString("fr-FR")}
                      </p>
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-3 sm:gap-4">
                    {transaction.status === "COMPLETED" && (
                      <Link
                        href={`/api/invoice/${transaction.id}`}
                        target="_blank"
                        download
                    ...
                    <span className={`status-badge ${transaction.status === "COMPLETED" ? "bg-emerald-500/10 text-emerald-600" : transaction.status === "PENDING" ? "bg-amber-500/10 text-amber-600" : "bg-red-500/10 text-red-600"}`}>
                      {transaction.status === "COMPLETED" ? "Réussi" : transaction.status === "PENDING" ? "En attente" : "Échoué"}
                    </span>
                  </div>
                </div>
              ))}
...
function PaymentStatusMessage({ status, locked }: { status?: string; locked?: string }) {
  if (status === "success") {
    return (
      <div className="rounded-lg border border-emerald-500/25 bg-emerald-500/10 p-4 text-sm font-bold leading-6 text-emerald-700 dark:text-emerald-400">
        Paiement confirmé. Votre solde est mis à jour automatiquement.
      </div>
    );
  }
  ...
  if (status === "pending") {
    return (
      <div className="rounded-lg border border-amber-500/25 bg-amber-500/10 p-4 text-sm font-bold leading-6 text-amber-700 dark:text-amber-400">
        Paiement en cours de vérification. Si le débit est confirmé, patientez quelques instants puis actualisez la page.
      </div>
    );
  }
  ...
  if (status === "error") {
    return (
      <div className="rounded-lg border border-red-500/25 bg-red-500/10 p-4 text-sm font-bold leading-6 text-red-700 dark:text-red-400">
        La confirmation du paiement a échoué. Aucun accès ne sera ouvert tant que le paiement n'est pas confirmé.
      </div>
    );
  }
