...
export default async function EnglishClubPage() {
    ...
    return (
      <div style={{ background:"#080808", minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"40px 20px" }}>
        <div style={{ maxWidth:560, textAlign:"center" }}>
          <div style={{ width:80, height:80, borderRadius:"50%", background:"rgba(231,22,42,0.1)", border:"1px solid rgba(231,22,42,0.2)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:36, margin:"0 auto 32px" }}>🔑</div>
          <div style={{ fontSize:11, color:"#E7162A", letterSpacing:"0.2em", textTransform:"uppercase", marginBottom:16 }}>Accès réservé</div>
          ...
          <Link href="/dashboard/student/courses" style={{ fontSize:13, fontWeight:700, letterSpacing:"0.12em", textTransform:"uppercase", background:"linear-gradient(135deg,#E7162A,#FF4D5E,#B30012)", color:"#080808", padding:"14px 32px", borderRadius:100, textDecoration:"none", display:"inline-block" }}>
            Reprendre mes cours →
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ paddingBottom:80 }}>
      ...
      {/* ── HERO ── */}
      <div style={{ position:"relative", overflow:"hidden", borderRadius:24, marginBottom:32, background:"linear-gradient(135deg, #0D0D14, #1A1520)", border:"1px solid #E7162A", padding:"48px 40px" }}>
        <div style={{ position:"absolute", top:-60, right:-60, width:300, height:300, borderRadius:"50%", background:"rgba(231,22,42,0.08)", filter:"blur(80px)", pointerEvents:"none" }}/>
        <div style={{ position:"absolute", top:0, left:0, right:0, height:1, background:"linear-gradient(90deg,transparent,#E7162A,transparent)", opacity:0.4 }}/>

        <div style={{ position:"relative", zIndex:1, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:24 }}>
          <div>
            <div style={{ fontSize:10, color:"#E7162A", letterSpacing:"0.2em", textTransform:"uppercase", marginBottom:14 }}>Espace Exclusif · Membre Actif</div>
            <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(2rem,4vw,3rem)", fontWeight:900, color:"#E7162A", marginBottom:12, lineHeight:1.1 }}>
              The English Club
            </h1>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12, alignItems:"flex-end" }}>
            <div style={{ border:"1px solid rgba(231,22,42,0.3)", borderRadius:12, padding:"12px 20px", textAlign:"center", background:"rgba(231,22,42,0.04)" }}>
              ...
              <div style={{ color:"#E7162A", fontWeight:700, fontSize:16 }}>B2 → C2</div>
            </div>
            <Link href="/dashboard/student/forum" style={{ fontSize:11, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", background:"linear-gradient(135deg,#E7162A,#FF4D5E,#B30012)", color:"#080808", padding:"12px 24px", borderRadius:100, textDecoration:"none", textAlign:"center" }}>
              Forum du Club →
            </Link>
          </div>
        </div>
      ...
      <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:40 }}>
        {["Chaleureux","Exigeant","Bienveillant","Chic","Networking","Global Mindset","Corrections légères","Feedback positif"].map(t => (
          <span key={t} style={{ fontSize:10, border:"1px solid rgba(231,22,42,0.2)", borderRadius:100, padding:"5px 14px", color:"rgba(245,240,232,0.5)", letterSpacing:"0.08em" }}>{t}</span>
        ))}
      </div>
      ...
      <div style={{ marginBottom:48 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(260px, 1fr))", gap:16 }}>
          {ACTIVITIES.map(({ icon, title, freq, desc }) => (
            <div key={title} style={{ border:"1px solid rgba(231,22,42,0.1)", borderRadius:16, padding:"28px 24px", background:"rgba(20,20,30,0.6)", backdropFilter:"blur(16px)", transition:"all 0.25s", cursor:"default" }}
              onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.borderColor="rgba(231,22,42,0.35)"; (e.currentTarget as HTMLElement).style.transform="translateY(-4px)";}}
              onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor="rgba(231,22,42,0.1)"; (e.currentTarget as HTMLElement).style.transform="translateY(0)";}}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
                <span style={{ fontSize:28 }}>{icon}</span>
                <span style={{ fontSize:9, color:"#E7162A", border:"1px solid rgba(231,22,42,0.3)", borderRadius:100, padding:"4px 10px", letterSpacing:"0.12em", textTransform:"uppercase" }}>{freq}</span>
              </div>
      ...
      {/* ── RÈGLES DU CLUB ── */}
      <div style={{ border:"1px solid rgba(231,22,42,0.15)", borderRadius:20, padding:"36px 32px", background:"rgba(231,22,42,0.03)" }}>
        <div style={{ fontSize:10, color:"#E7162A", letterSpacing:"0.2em", textTransform:"uppercase", marginBottom:20 }}>Règles du Club</div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))", gap:20 }}>
          {[
            ...
            <div key={n}>
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:32, fontWeight:900, color:"rgba(231,22,42,0.2)", lineHeight:1, marginBottom:10 }}>{n}</div>
