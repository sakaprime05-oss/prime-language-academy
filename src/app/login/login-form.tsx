    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {paymentStatus === "payment_success" && (
        <div className="p-4 text-xs font-bold text-emerald-500 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-center animate-in fade-in slide-in-from-top-1">
          Paiement reçu. Vous pouvez maintenant vous connecter à votre espace.
        </div>
      )}

      {paymentStatus === "payment_pending" && (
        <div className="p-4 text-xs font-bold text-amber-500 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-center animate-in fade-in slide-in-from-top-1">
          Paiement en cours de vérification. Réessayez dans quelques instants ou contactez l'administration.
        </div>
      )}

      {paymentStatus === "payment_error" && (
        <div className="p-4 text-xs font-bold text-red-500 bg-red-500/10 border border-red-500/20 rounded-2xl text-center animate-in fade-in slide-in-from-top-1">
          La confirmation du paiement n'a pas pu être vérifiée. Contactez l'administration si le débit est confirmé.
        </div>
      )}
